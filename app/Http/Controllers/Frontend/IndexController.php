...
class IndexController extends Controller
{
    ...
    public function VendorDetails($id)
    {
        ...
        $vendor_name = User::select('name')
            ->where('id', $id)
            ->get();
        $vproduct = Product::where('vendor_id', $id)->get();
        return view('frontend.vendor.vendor_details', compact('vendor', 'vproduct', 'id', 'vendor_name'));
    } // End Method
    ...
    public function vendorrating(Request $request, $id)
    {
        $vendor = User::findOrFail($id);
        $vendor->name;
        $rate = $request->rate;
        ...
        $vendor_count = vendor_rating::where('vendor_id', $id)->count();

        $rating_vendor = vendor_rating::where('vendor_id', $id)->sum('rating');

        $avg_rating = $rating_vendor / $vendor_count;

        User::findOrFail($id)->update([
            'vendor_rating' => $avg_rating,
        ]);
        // $r_vendor = User::select('vendor_rating')
            // ->where('id', $id)
            // ->first();
        return view('frontend.vendor.vendor_details', compact('vendor', 'vproduct', 'id', 'vendor_name'));
